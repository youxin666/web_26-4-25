...
assert.match(worker, /href="\/about" role="menuitem"><svg class="user-menu-item-icon"[\s\S]*?data-i18n="nav\.aboutSite">关于本站<\/span><\/a>/);
assert.match(client, /'nav\.aboutSite': '关于本站'/);
assert.match(client, /'nav\.aboutSite': 'About this site'/);
...
assert.match(worker, /class="hidden" data-profile-avatar-hint data-i18n="profile\.avatarHint"/);
...
assert.match(client, /var avatarHint = root\.querySelector\('\[data-profile-avatar-hint\]'\)/);
assert.match(client, /if \(avatarHint\) avatarHint\.classList\.toggle\('hidden', !isEditing\)/);
