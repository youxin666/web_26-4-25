...
assert.match(script, /'myArticles\.deleted': '草稿已删除，剩余 \{count\} 篇草稿。'/);
assert.match(script, /deleteStatus\.textContent = t\('myArticles\.deleted'\)\.replace\('\{count\}', String\(groups\.draft\.length\)\)/);
