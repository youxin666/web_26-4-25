...
assert.match(css, /\.site-header::after\s*\{[^}]*width:\s*100vw;[^}]*height:\s*100%;[^}]*background:\s*linear-gradient\(90deg,[^}]*backdrop-filter:\s*blur\(44px\) saturate\(160%\);[^}]*mask-image:\s*linear-gradient\(90deg,/s, 'both viewport edges need an extra feathered blur to remove hard cut lines');
assert.match(css, /:root\[data-theme="dark"\] \.site-header::after\s*\{[^}]*background:\s*linear-gradient\(90deg,/s, 'the stronger edge blur needs a dark-mode tint');
...
for (const name of fs.readdirSync(new URL('../public/', import.meta.url)).filter((file) => file.endsWith('.html'))) {
  ...
  assert.match(html, /<link rel="stylesheet" href="fixed-header\.css\?v=20260908-mobile-main-divider">/, `${name} must load the fixed header override`);
}
