...
assert.match(css, /\.site-header::before\s*\{[^}]*position:\s*absolute;[^}]*top:\s*-12px;[^}]*left:\s*50%;[^}]*width:\s*100vw;[^}]*height:\s*calc\(100% \+ 12px\);[^}]*transform:\s*translateX\(-50%\);[^}]*backdrop-filter:\s*blur\(24px\) saturate\(1\.45\);/s, 'the glass backdrop must fill the top and both side gaps');
...
assert.match(css, /@media \(max-width:\s*640px\)[\s\S]*?\.site-header::before\s*\{[^}]*height:\s*calc\(100% \+ 8px\);/s, 'the mobile glass backdrop must also fill the top and side gaps');
...
for (const name of fs.readdirSync(new URL('../public/', import.meta.url)).filter((file) => file.endsWith('.html'))) {
  ...
  assert.match(html, /<link rel="stylesheet" href="fixed-header\.css\?v=20260907-immersive-header-sides">/, `${name} must load the fixed header override`);
}
